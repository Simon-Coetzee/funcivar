...
#'   files need to be BED 3 + 2 format. Column 4 contains the sample name,
#'   Column 5 contains the chromatin state. And they need to be completely
#'   disjoint intervals.
#'
#' @param genome A character vector describing the genome build against which
    ...
    bed.list <- lapply(files,
                       function(file, s.info) {
                         col.types <- cols_only(chr = col_character(),
                                                ...
                                                state = col_character())
                         col.names <- c("chr", "start", "end", "sample", "state")
                         col.numbers <- c(1:5)
                         if (any(grepl(".gz$", file))) {
                           xf <- suppressMessages(suppressWarnings(read_tsv(file = file, col_names = col.names,
                                                                            col_types = col.types,
...
#'   feature overlaps, or "segmenations" specifying that the features slot is
...
CalculateEnrichment <- function(variants, features, feature.type = "biofeatures",
  ...
  if (feature.type == "biofeatures") {
  ...
  } else if (feature.type == "segmenations") {
    if (missing(features)) {
    ...
    stop("feature.type: ", feature.type, "is not availible; try 'biofeatures' or 'segmenations'")
  }
  return(enrichment)
}
...
SetOverlaps <- function(variants, features) {
  ...
  overlaps <- data.frame(from = names(variants)[from(overlaps)],
                         to = names(features)[to(overlaps)],
                         stringsAsFactors = FALSE)
  if (sum(!(names(variants) %in% overlaps$from)) > 0L) {
    missing.variants <- data.frame(from = names(variants)[!(names(variants) %in% overlaps$from)],
                                   to = NA,
                                   stringsAsFactors = FALSE)
    overlaps <- rbind(overlaps, missing.variants)
  }
  if (sum(!(names(features) %in% overlaps$to)) > 0L) {
    missing.features <- data.frame(from = NA,
                                   to = rep.int(names(features)[!(names(features) %in% overlaps$to)], length(names(variants))),
                                   stringsAsFactors = FALSE)
    overlaps <- rbind(overlaps, missing.features)
  }
  resmatrix <- make.overlap.matrix(overlaps)[names(variants), ]
  if(is(variants, "GRanges")) {
    ...
    overlap.offset <- ncol(rowData(variants)) + 1
    mcols(rowRanges(variants)) <- cbind(rowData(variants), DataFrame(resmatrix))
...
ShowOverlaps <- function(variants, feature = NULL) {
  ...
  if (is.null(feature)) {
    ...
    if (any(feature %in% colnames(rowData(variants)))) {
...
#' Plot Enrichment
#' @description a flexible plotting tool of the enrichment data.frame generated by CalculateEnrichment()
#' @param variant.enrichment a data.frame of enrichment data generated by CalculateEnrichment()
#'
#' @param value one of 'difference' or 'oddsratio', thereby plotting the difference in beta distributions between foreground and background, or the oddsratio of the comparison.
#' @param block1 character string defining the column of variant.enrichment to be used as a factor by which to separate the enrichment into blocks (separate panels) in the plot. Default is NULL, in which case there is no blocking.
#' @param block2 character string defining the column of variant.enrichment to be used as a factor by which to separate the enrichment into blocks (separate panels) in the plot. Default is NULL, in which case there is no blocking.
#' @param color.by character string defining the column of variant.enrichment to be used to color the values.
#' @param ncol number of columns to use for facet_wrap if only one block is defined.
#'
...
#' @export
PlotEnrichment <- function(variant.enrichment, value = "difference", block1 = NULL, block2 = NULL, color.by = NULL, ncol = 1) {
  ...
  if (!(value %in% c("oddsratio", "difference"))) {
    stop("The y argument must be either 'oddsratio' or 'difference'")
  }
  ...
  if (is.null(color.by)) {
    ...
    color.name <- color.by
    if (is.character(color.vals) | is.factor(color.vals) | is.logical(color.vals)) {
      ...
      variant.enrichment[!variant.enrichment$significant, "color"] <- alpha(variant.enrichment[!variant.enrichment$significant, "color"], 0.2)
      color.is <- "self"
    } else {
  ...
  if (n_distinct(variant.enrichment$sample) <= 5L) {
    if(value == "oddsratio") {
      ep <- ep + geom_crossbar(aes(ymin = odds.lower, ymax = odds.upper, color = color), fatten = 1.2, size = 2)
    } else {
      ep <- ep + geom_crossbar(aes(ymin = lower, ymax = upper, color = color), fatten = 1.2, size = 2)
    }
  } else {
    if(value == "oddsratio") {
      ep <- ep + geom_pointrange(aes(ymin = odds.lower, ymax = odds.upper, color = color), fatten = 0.5)
    } else {
  ...
  if(value == "oddsratio") {
    ...
    ep <- ep + geom_hline(yintercept = 1, color = "#c4c4c4", alpha = 0.5) +
  ...
  ep <- ep + scale_x_discrete(name = "Sample") +
    scale_y_continuous(name = value, breaks = round(seq(from = my.min, to = my.max, length.out = 5), 2))
  if (all(!is.null(block1), !is.null(block2))) {
    ...
    ep <- ep + theme(axis.text.x = element_text(angle = 270),
                     legend.position="bottom",
                     panel.grid.major.x = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(),
                     strip.text.x = element_text(angle = 0, hjust = 0.5, vjust = 1, size = rel(1.5)))
  }
...
enrich.features <- function(fg, bg, CI, prior, strict.subset) {
  ...
  } else if (metadata(fg)$source == "funciVar") {
    ## foreground overlaps
